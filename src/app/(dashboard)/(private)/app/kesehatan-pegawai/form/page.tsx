...
import {
  fetchKesehatanSantriById,
  postKesehatanSantri,
  putKesehatanSantriUpdate,
  resetRedux
} from '../../kesehatan-santri/slice/index'
...
const KesehatanPegawaiForm = () => {
  ...
  const initForm = useCallback(async () => {
    try {
      const [resPegawai, resLocation] = await Promise.all([
      ...
      const allLocations = resLocation?.data?.values || []

      const kamarOptions = allLocations
        .filter((item: any) => item.jenis_lokasi === 'Kamar')
        .map((item: any) => ({
          label: `${item.nama_lokasi} (${item.parent?.nama_lokasi || 'Asrama'})`,
          value: item.id_lokasi
        }))

      const lokasiKerjaOptions = allLocations.map((item: any) => ({
        label: `${item.nama_lokasi} (${item.jenis_lokasi || 'Lokasi'})`,
        value: item.id_lokasi
      }))
      ...
      setOpt({
        ...
        kamar: kamarOptions,
        lokasiKerja: lokasiKerjaOptions
      })

      if (id) {
        // 2. Fetch Detail Kesehatan
        ...
        if (d) {
          const formatted = {
            ...d,
            id_pegawai: pegawaiOptions.find((o: any) => o.value === d.id_pegawai) || null,
            ...
            id_lokasi_kerja: lokasiKerjaOptions.find((o: any) => o.value === d.id_lokasi_kerja) || null,
  ...
  useEffect(() => {
    if (store.crud?.status) {
      toast.success(store.crud?.message || 'Data pemeriksaan pegawai berhasil disimpan')
      dispatch(resetRedux())
      router.replace('/app/kesehatan-pegawai/list')
  ...
  const onSubmit = () => {
    ...
    const pegawaiId = state.id_pegawai?.value || state.id_pegawai
  ...
  const getIdentitasFields = () => {
    return [
      field({
        ...
        placeholder: 'Pilih Pegawai / Guru',
        options: { values: opt.pegawai },
        required: true,
        readOnly: !!view || !!id,
        onChange: (val: any) => {
          const newState = { ...state, id_pegawai: val }
          setState(newState)
          reset(newState)
        }
      }),
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
        <IconButton component={Link} href='/app/kesehatan-pegawai/list'>
          <i className='tabler-arrow-left' />
        </IconButton>
        <Box>
          <Typography variant='h5' sx={{ fontWeight: 600 }}>
            {id ? (view ? 'Detail Pemeriksaan Pegawai' : 'Edit Pemeriksaan Pegawai') : 'Pemeriksaan Kesehatan Pegawai'}
          </Typography>
          <Typography variant='body2' color='text.secondary'>
            {id && view ? 'Detail data pemeriksaan kesehatan pegawai' : 'Input data pemeriksaan kesehatan pegawai'}
          </Typography>
        </Box>
      </Box>
            ...
            {formColumn({
              ...
              fields: [
                fieldBuildSubmit({
                  submit: 'Simpan',
                  cancel: 'Batal',
                  onCancel: () => router.push('/app/kesehatan-pegawai/list'),
...
export default KesehatanPegawaiForm
