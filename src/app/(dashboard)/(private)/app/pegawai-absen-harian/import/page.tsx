...
export interface ImportJamKerjaRow {
  row: number
  valid: boolean
  error: string | null
  payload: ImportJamKerjaPayload
}

export interface ImportJamKerjaPreviewResponse {
  mode: 'preview' | 'commit'
  total: number
  valid: number
  invalid: number
  data: ImportJamKerjaRow[]
}

interface Props {
  result: ImportJamKerjaPreviewResponse
  onCommit: () => void
}

export default function ImportExcelJamKerjaPegawaiPage() {
  ...
  const [preview, setPreview] = useState<ImportJamKerjaPreviewResponse>()

  /** Navigasi kembali ke list jam kerja pegawai */
  ...
  const downloadTemplate = async () => {
    try {
      const res = await dispatch(postJamKerjaExport({ template: '1' })).unwrap()
  ...
  const onSubmit = async () => {
    ...
    try {
      setLoading(true)
      const res = await dispatch(postJamKerjaImport(formData)).unwrap()
      ...
      if (mode === 'preview') {
        ...
        toast.success('Import data jam kerja berhasil')
        onCancel()
      }
    } catch (e) {
      console.error(e)
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const res = await dispatch(postJamKerjaInsertBatch({ data: payloads })).unwrap()
      ...
      toast.success('Import data jam kerja pegawai berhasil')
      onCancel()
    } catch (e) {
      console.error(e)
          ...
          <table className='min-w-full text-xs whitespace-nowrap'>
            <thead className='bg-gray-100 text-left border-b'>
              <tr>
                ...
                <th className='px-3 py-2'>ID / Ref Pegawai</th>
                <th className='px-3 py-2'>ID / Ref Lokasi</th>
                <th className='px-3 py-2'>Jam Masuk (Mulai)</th>
                <th className='px-3 py-2'>Jam Pulang (Selesai)</th>
                <th className='px-3 py-2'>Status Keaktifan</th>
                <th className='px-3 py-2'>Keterangan / Memo</th>
            ...
            <tbody className='divide-y divide-gray-100'>
              {result.data.map(row => (
                <tr key={row.row} className={row.valid ? 'hover:bg-gray-50' : 'bg-red-50'}>
                  ...
                  {/* 2. ID Pegawai */}
                  <td className='px-3 py-2 font-mono font-semibold text-gray-700'>{row.payload.id_pegawai || '-'}</td>

                  {/* 3. ID Lokasi */}
                  <td className='px-3 py-2 font-mono font-semibold text-gray-700'>{row.payload.id_lokasi || '-'}</td>

                  {/* 4. Waktu Mulai */}
                  <td className='px-3 py-2 font-medium text-blue-600'>{row.payload.waktu_mulai || '-'}</td>

                  {/* 5. Waktu Selesai */}
                  <td className='px-3 py-2 font-medium text-red-600'>{row.payload.waktu_selesai || '-'}</td>

                  {/* 6. Status Keaktifan */}
                  <td className='px-3 py-2'>
                    <span
                      className={`font-semibold ${
                        String(row.payload.is_active) === 'true' || row.payload.is_active === true
                          ? 'text-green-600'
                          : 'text-gray-400'
                      }`}
                    >
                  ...
                  {/* 7. Keterangan */}
                  <td className='px-3 py-2 whitespace-normal min-w-[200px] leading-tight text-gray-600'>
  ...
  return (
    <div className='min-h-screen bg-gray-50 py-10 px-4'>
      <div className='max-w-4xl mx-auto'>
        <div className='mb-6'>
          <Link
            href='/app/pegawai-jam-kerja/list'
            className='inline-flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900'
          >
            <i className='tabler-arrow-back-up'></i> Kembali ke List Jam Kerja
          </Link>
          <h1 className='text-xl font-semibold mt-2'>Import Data Jam Kerja Pegawai</h1>
        </div>

        <div className='bg-white rounded-xl shadow-sm border p-6 space-y-6'>
          <div className='border border-blue-200 bg-blue-50 rounded-lg p-4 text-sm text-blue-700'>
            <h3 className='font-medium mb-2 font-bold'>Petunjuk Import Jam Kerja</h3>
            <ul className='list-disc list-inside space-y-1 opacity-90'>
              <li>Gunakan format file Excel (.xlsx) sesuai template acuan jam kerja.</li>
              <li>
                Pastikan <b>ID Pegawai</b> dan <b>ID Lokasi</b> valid serta sudah terdaftar di sistem.
              </li>
              <li>Gunakan format waktu 24 jam bersih (Contoh: 08:00 atau 17:00).</li>
              <li>
                Nilai kolom status keaktifan diisi dengan <code>true</code> atau <code>false</code>.
              </li>
            </ul>
            <div className='mt-3'>
              <a className='text-blue-600 underline hover:text-blue-400 cursor-pointer' onClick={downloadTemplate}>
                <b>Download Template Excel Jam Kerja</b>
              </a>
            </div>
          </div>
