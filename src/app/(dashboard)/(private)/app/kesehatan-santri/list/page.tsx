  ...
  const content = (
    <>
      ...
      >
        <MenuItem
          component={Link}
          href={`/app/kesehatan-santri/form?id=${row.id_kesehatan}&view=true`}
          onClick={handleClose}
        >
      ...
      <DialogDelete
        id={row.santri?.fullname || 'Pemeriksaan Kesehatan'}
        open={openConfirm}
  ...
  const onExport = async () => {
    try {
      ...
      if (res?.status && res?.data) {
        ...
        toast.success('Data kesehatan santri berhasil diexport')
      } else {
  ...
  useEffect(() => {
    if (store.delete) {
      if (store.delete.status) {
        toast.success(store.delete.message || 'Log medis berhasil dihapus')
        fetchData()
      } else {
    ...
    <Grid container spacing={6}>
      <Grid size={12}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
          <Box>
            <Typography variant='h5'>Kesehatan Santri</Typography>
            <Typography variant='body2' color='text.secondary'>
              Modul Kesehatan Santri
            </Typography>
          </Box>
        </Box>
          ...
          {canCreate && (
            <Tooltip title='Pemeriksaan Baru'>
              <Button
                ...
                component={Link}
                href='/app/kesehatan-santri/form'
...
export default KesehatanSantriList
